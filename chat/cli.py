...
def run_repl() -> None:
    session = TSChatSession()
    print("TS-Chat v0.5 (TSLC). Type 'quit' to exit, 'state' for JSON state, 'trace' for last receipt.\n")
    while True:
        try:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="TS-Chat v0.5 — TSLC conversational shell")
